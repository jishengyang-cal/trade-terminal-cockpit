...
| TUI command palette | Command palette shows gateway configuration, command evidence, authority status, audit status, and replayable command-envelope forms. | Closed | VM compile/smoke |
...
| TUI command roundtrip | Strategy, order, and risk keybindings open exact-confirmation modals, submit `CommandEnvelope` requests through `command-gateway`, read produced authority/audit events, and reduce those events into the cockpit state. | Closed | VM command-gateway/TUI smoke and local copied-binary gateway smoke |
