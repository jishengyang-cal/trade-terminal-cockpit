...
| Command System | `tradectl` emits replayable command envelopes; `command-gateway` validates, applies capability policy, and records audit events. | Closed | VM smoke accepted/rejected gateway assertions |
...
| command-gateway | Standalone command gateway boundary exists outside `trade-tui`. | Closed | VM smoke `command-gateway --command-json` |
| authority / risk engine integration | Gateway has an explicit capability/authority policy and denies dangerous commands by default; broker/risk execution remains out of repo. | Closed | VM smoke dangerous and bad-capability rejection |
...
| Flatten / kill switch backend chain | `tradectl` emits dangerous envelopes, TUI previews them, and gateway audits/rejects by default; broker execution remains outside this repo. | Closed | VM smoke dangerous rejection |
